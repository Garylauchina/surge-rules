...
Read this first when working in this repository. This file is the project-local
context for future agents so the network structure does not need to be
re-explained every session.

This repository is the public-safe source for shared rule intent and maintenance
notes. Do not commit proxy subscriptions, API tokens, controller secrets,
dashboard passwords, generated R2 object keys, or private device credentials.

## Architecture

The proxy system has one public rule source, one hosted configuration delivery
layer, three private VPS endpoints, and two active clients.
...
Verified on 2026-07-24 after retiring the non-working Reality services.
...
R4S OpenClash at home
  ...
  routing.
  Current first home exit: Vultr LA Hysteria2 on UDP 8443, with Vultr Trojan
  on TCP 8443 as the same-host fallback.
...
iPhone Surge away profile
  ...
  8443. The phone profile did not use Reality and required no retirement change.

Vultr LA VPS
  Public host: vps.prometheusclothing.net
  Address: 45.32.87.131
  Services: Trojan TLS 8443 and Hysteria2 8443.
  Reality TCP 443 was retired on 2026-07-24.
...
2026-07-23 after the Vultr service recovery:
...
The active R4S profile uses ordered fallback groups and does not use load
balancing. The retired Vultr and Bandwagon Reality nodes are no longer present
in the profile.

Cold-standby hardening verified on 2026-07-23:

- Vultr is the first daily and US exit, Bandwagon is second, and Tokyo is the
  low-latency manual/last fallback path.
...
- Service groups should default to the ordered private VPS paths and expose
  红杏 only as a deliberate manual emergency choice.
...
- The R2-hosted profile includes Vultr Hysteria2 and Trojan as automatic paths,
  excludes the retired Reality nodes, and includes the IPPure/DNS/WebRTC
  leak-test routing fixes.
...
Steam login and account-recovery backends need real DNS answers. Keep
`+.steamserver.net` and `steamconnecttest.com` in
`OpenClashFakeIPFilter.list`, and keep their narrow domestic resolver policy in
the generated R4S profile.
