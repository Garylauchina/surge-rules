...
`SharedRules.dconf` is the shared `[Rule]` source used to keep Mac, iOS, and
...
Generated or hosted profiles should keep the same `[Rule]` content as
`SharedRules.dconf`:

```ini
[Rule]
# same rule order as SharedRules.dconf
```
